...
You can install the development version of STOPeData from the [GitHub
Repository](https://github.com/NIVANorge/STOPeData) with the
`{devtools}` package:
...
#> Skipping install of 'STOPeData' from a github remote, the SHA1 (747f3049) has not changed since last install.
#>   Use `force = TRUE` to force installation
```

This will also install [eDataDRF](github.com/NIVANorge/eDataDRF), which
defines the format. Run the Shiny application as below.
...
Some functionality will be unavailable until you add the appropriate API
keys/variables to your `.Renviron` file.
